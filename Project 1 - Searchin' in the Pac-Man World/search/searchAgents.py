...
def foodHeuristic(state, problem):
  ...
  """
  # The heuristic below is not admissible.
  
  from util import manhattanDistance
  remainingFood = foodGrid.asList()
  totalDist = 0

  # return 0 if the agent is in the goal state
  if problem.isGoalState(state): return totalDist

  # find distance (manhattan distance) to the closest remaining food
  # from the current state
  closestFood = remainingFood[0]
  distToClosest = manhattanDistance(position, closestFood)
  for food in remainingFood[1:]:
    dist = manhattanDistance(position, food)
    if dist < distToClosest:
      closestFood = food
      distToClosest = dist
  # remove the food we just found from the list and update the total distance
  remainingFood.remove(closestFood)
  totalDist += distToClosest

  # then find distance to the cloest remaining food from the food we just found,
  # and keep doing so until we get the total distance to all food
  while len(remainingFood) != 0:
    nextFood = remainingFood[0]
    distToClosest = manhattanDistance(closestFood, nextFood)
    for food in remainingFood[1:]:
      dist = manhattanDistance(closestFood, food)
      if dist < distToClosest:
        nextFood = food
        distToClosest = dist

    # update the remaining food list, current food and the total distance
    remainingFood.remove(nextFood)
