...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    "*** YOUR CODE HERE ***"
    # stores beliefs of the current state for updating self.beliefs
    allPossible = util.Counter()
    for p in self.legalPositions:
      trueDist = util.manhattanDistance(p, pacmanPosition)
      if emissionModel[trueDist] > 0:
        allPossible[p] = emissionModel[trueDist] * self.beliefs[p]
    allPossible.normalize()
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    pacmanPosition = gameState.getPacmanPosition()
    "*** YOUR CODE HERE ***"
    weightParticles = util.Counter()
    for p in self.particles:
      trueDist = util.manhattanDistance(p, pacmanPosition)
      if emissionModel[trueDist] > 0:
        weightParticles[p] += emissionModel[trueDist]

    resampledParticles = []
    if weightParticles.totalCount() == 0:
      for i in range(self.numParticles):
        resampledParticles += [random.choice(self.legalPositions)]
    else:
      for i in range(self.numParticles):
        resampledParticles += [util.sample(weightParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.particles:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      resampledParticles += [util.sample(newPosDist)]
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      for i in range(self.numGhosts):
        newParticle[i] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)),
                                                                     i + 1, self.ghostAgents[i]))
  ...
  def observeState(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    weightParticles = [util.Counter() for i in range(self.numGhosts)]

    for p in self.particles:
      for i in range(self.numGhosts):
        trueDist = util.manhattanDistance(p[i], pacmanPosition)
        weightParticles[i][p[i]] += emissionModels[i][trueDist]

    if sum([weightParticles[i].totalCount() for i in range(self.numGhosts)]) == 0:
      self.initializeParticles()
    else:
      resampledParticles = []
      for i in range(self.numParticles):
        ...
        for ghostIdx in range(self.numGhosts):
          if noisyDistances[ghostIdx] == 999:
            pos = tuple([2 * ghostIdx + 1, 1])
            newParticle.append(pos)
          else:
            newParticle.append(util.sample(weightParticles[ghostIdx]))
